...
local function RequestAllGearItems(gearData)
    ...
    -- And Wowhead gear items for the same class/spec
    if selectedClass and selectedSpec and ns.GetWowheadGearSpecData then
        local whData = ns:GetWowheadGearSpecData(selectedClass, selectedSpec)
        if whData and whData.bisGear then
            for _, tab in ipairs(whData.bisGear) do
                for _, g in ipairs(tab.slots) do RequestItemData(g.item.itemId) end
            end
        end
    end
...
local function InitFrame()
    ...
    local function SourceLabel(source)
        ...
        if source == "wowhead" then return SOURCE_ICON_WOWHEAD end
    ...
    UI._refreshTalentSourceDropdown = function()
        ...
        -- Wowhead is the exception: it only publishes talent builds for some
        -- specs, so it's offered per-spec rather than unconditionally. Without
        -- this guard the option would appear and then render an empty pane.
        if ns.HasWowheadTalents and ns:HasWowheadTalents(selectedClass, selectedSpec) then
            opts[#opts + 1] = { label = SOURCE_ICON_WOWHEAD, value = "wowhead" }
        elseif UI.talentSource == "wowhead" then
            UI.talentSource = "icyveins"
        end
        -- Archon is per-spec for the same reason.
        if ns.HasArchonTalents and ns:HasArchonTalents(selectedClass, selectedSpec) then
            opts[#opts + 1] = { label = SOURCE_ICON_ARCHON, value = "archongg" }
        elseif UI.talentSource == "archongg" then
            UI.talentSource = "icyveins"
        end
...
local function UpdateCompendiumAttribution(class, spec)
    ...
    elseif activeTab == "talents" then
        ...
        elseif ts == "wowhead" then key, url = "wowhead", U.wowheadTalents(class, spec) or ns.SOURCES.wowhead.homepage
        elseif ts == "archongg" then key, url = "archongg", U.archonTalents(class, spec) or ns.SOURCES.archongg.homepage
    ...
    elseif activeTab == "enhancements" then
        ...
        -- Follow the picked source rather than always linking u.gg. Wowhead's
        -- enchant page has a role suffix the scraper resolves and never guesses,
        -- and no per-spec link is stored for it, so Wowhead attributes to its
        -- bis-gear page — the one link that always resolves.
        if key == "wowhead" then url = U.wowheadGear(class, spec) or ns.SOURCES.wowhead.homepage
        elseif key == "archongg" then url = U.archonEnchants(class, spec) or ns.SOURCES.archongg.homepage
        else url = U.uggOverview(class, spec) or ns.SOURCES.ugg.homepage end
...
-- group by hero rather than by context the way Icy Veins does —
-- ns.GroupBuildsByHero already models exactly that, and both accessors hand back
-- the canonical build shape, so this stays a straight render with no reshaping.
-- The two sources differ only in where the builds come from and what the empty
-- state says, so they share one renderer.
local function RenderHeroGroupedTalentList(builds, emptyText, yPos)
    if not builds or #builds == 0 then
        UI.talentFallback:SetText(emptyText)
        UI.talentFallback:ClearAllPoints()
        UI.talentFallback:SetPoint("TOPLEFT", UI.talentContent, "TOPLEFT", 4, -(yPos + 4))
        UI.talentFallback:Show()
        return yPos + 20
    ...
    for _, hero in ipairs(heroOrder) do
        ...
        for _, build in ipairs(group) do
            rowIdx = rowIdx + 1
            local btn = UI._ensureTalentButton(rowIdx)
            if btn.heroIcon then btn.heroIcon:Hide() end
            btn.label:ClearAllPoints()
...
local function RenderWowheadTalentList(class, spec, yPos)
...
function ns:UpdateCompendiumEnchants()
    ...
    local whEnh = ns.GetWowheadEnhancements and ns:GetWowheadEnhancements(selectedClass, selectedSpec)
    local archonEnh = ns.GetArchonEnhancements and ns:GetArchonEnhancements(selectedClass, selectedSpec)
    ns.Sections.Enhancements.RenderCompendiumEnchantsGems({
        ...
        whEnchants     = whEnh and whEnh.enchants,
        whGems         = whEnh and whEnh.gems,
        archonEnchants = archonEnh and archonEnh.enchants,
        archonGems     = archonEnh and archonEnh.gems,
...
function ns:UpdateCompendiumConsumables()
    ...
    ns.Sections.Enhancements.RenderCompendiumConsumables({
        ...
        whConsumables     = whEnh and whEnh.consumables,
        archonConsumables = archonEnh and archonEnh.consumables,
    })
end
...
function ns:UpdateCompendiumBis()
    ...
    local uggSpecData = ns.GetUggGearSpecData and ns:GetUggGearSpecData(selectedClass, selectedSpec)
    local whSpecData = ns.GetWowheadGearSpecData and ns:GetWowheadGearSpecData(selectedClass, selectedSpec)
    ...
    ns.Sections.Gear.RenderCompendium({
        ...
        whBis      = whSpecData and whSpecData.bisGear,
