...
function ns:UpdatePanel()
    ...
    -- Rotation source: Icy Veins (the legacy ClassCodexData path) or Wowhead.
    -- Only offered where both actually have steps for this spec, so the picker
    -- never appears with one entry or lands the player on an empty list.
    local whRotation = ns.GetWowheadRotation and ns:GetWowheadRotation(classToken, specKey) or nil
    local hasIvRotation = specData.rotation and #specData.rotation > 0
    local rotSourceOptions = {}
    if hasIvRotation then rotSourceOptions[#rotSourceOptions + 1] = "Icy Veins" end
    if whRotation then rotSourceOptions[#rotSourceOptions + 1] = "Wowhead" end

    local currentRotationSource = (perSpecRot and perSpecRot.rotationSource) or rotSourceOptions[1]
    local sourceStillValid = false
    for _, s in ipairs(rotSourceOptions) do
        if s == currentRotationSource then sourceStillValid = true; break end
    end
    if not sourceStillValid then currentRotationSource = rotSourceOptions[1] end

    local rotation
    if currentRotationSource == "Wowhead" then
        rotation = whRotation
        -- Wowhead's list isn't split by context, so the context picker is moot.
        rotCtxOptions = {}
    else
        rotation = FindRotationByContext(specData.rotation, currentHeroTalent, currentRotationContext)
    end

    lastRotationContentHeight = ns.Sections.Rotation.RenderPanel({
        sourceOptions  = rotSourceOptions,
        currentSource  = currentRotationSource,
        onSourceChange = function(picked)
            local ps = GetPerSpecState()
            if not ps then return end
            ps.rotationSource = picked
            ns:UpdatePanel()
        end,
...
function ns:LayoutPanel()
    ...
    if rotationSection:IsShown() and not rotationSectionCollapsed then
        ...
        if ns.Sections.Rotation.IsPanelSrcDropdownShown() then
            rotationContentHeight = rotationContentHeight + 30
        end
...
-- The BiS sources the item tooltip can show, in display order.
--
-- File-scope and keyed by *setting name* rather than by the setting's value:
-- three places need this list (the "is anything enabled" gate, the header
-- legend, and the per-spec rows), and the value has to be re-read on every
-- tooltip so a toggle takes effect without a reload.
--
-- `showTabs`  — source splits its picks by content type, so show which.
-- `showPct`   — source publishes an adoption share (Archon only).
-- `skipOnTrinket` — suppressed once a trinket tier row is shown; the tier
--                   already says everything useful.
local BIS_TOOLTIP_SOURCES = {
    { key = "ugg", setting = "showUggBisTooltip", getter = "GetUggBisSpecs",
      tex = "ugg", abbr = "u.gg", hex = "|cffb07cff" },
    { key = "icyveins", setting = "showIcyVeinsBisTooltip", getter = "GetIcyVeinsBisSpecs",
      tex = "icyveins", abbr = "IV", hex = "|cff00ccff", showTabs = true, skipOnTrinket = true },
    { key = "wowhead", setting = "showWowheadBisTooltip", getter = "GetWowheadBisSpecs",
      tex = "wowhead", abbr = "WH", hex = "|cffffa030", skipOnTrinket = true },
    { key = "archongg", setting = "showArchonBisTooltip", getter = "GetArchonBisSpecs",
      tex = "archon", abbr = "Archon", hex = "|cffd94f4f", showTabs = true, showPct = true,
      skipOnTrinket = true },
}

-- Brand chip for one source: icon, label, or both, per tooltipSourceStyle.
...
-- True when at least one BiS source is switched on. The tooltip hook gates on
-- this, so a source missing from the list here would render nothing however it
-- was configured.
local function AnyBisSourceEnabled()
    for _, src in ipairs(BIS_TOOLTIP_SOURCES) do
        if ClassCodexDB[src.setting] then return true end
    end
    return false
end
...
local function BuildTooltipEntries(itemId)
    ...
    -- BiS gear — merged per-spec across every enabled source.
    -- label → { class, spec, consolidated, bySource = { [key] = { tabs, popularity } } }
    local bisBySpec = {}
    local bisOrder = {}

    for _, src in ipairs(BIS_TOOLTIP_SOURCES) do
        local getter = ns[src.getter]
        if ClassCodexDB[src.setting] and getter and not (src.skipOnTrinket and hasTrinketEntries) then
            local specs = getter(ns, itemId)
            if specs then
                for _, entry in ipairs(specs) do
                    if not filterSet or filterSet[entry.class] then
                        local existing = bisBySpec[entry.label]
                        -- `seen` holds labels already emitted as trinket rows;
                        -- never start a new row for one, but do let a source
                        -- merge into a row another source already opened.
                        if not existing and not seen[entry.label] then
                            existing = { class = entry.class, spec = entry.spec,
                                         consolidated = entry.consolidated, bySource = {} }
                            bisBySpec[entry.label] = existing
                            bisOrder[#bisOrder + 1] = entry.label
                        end
                        if existing then
                            local rec = existing.bySource[src.key]
                            if not rec then
                                rec = { tabs = {} }
                                existing.bySource[src.key] = rec
                            end
                            for _, t in ipairs(entry.tabs or {}) do
                                rec.tabs[#rec.tabs + 1] = t
                            end
                            if entry.popularity then rec.popularity = entry.popularity end
                        end
                    end
                end
            end
    ...
    for _, label in ipairs(bisOrder) do
        if not seen[label] then
            ...
            if next(info.bySource) then
                local style = ClassCodexDB.tooltipSourceStyle or 1
                local showIcon = style == 1 or style == 3
                local showLabel = style == 2 or style == 3
                local parts = {}
                for _, src in ipairs(BIS_TOOLTIP_SOURCES) do
                    local rec = info.bySource[src.key]
                    if rec then
                        -- Icon here, not the full chip: the label is conditional
                        -- on there being no tab context to show in its place.
                        local s = showIcon and BisSourceChip(src, true, false) or ""
                        -- Deduplicate tabs
                        local uniqueTabs, tabSeen = {}, {}
                        ...
                        -- Show tab context when not all tabs. Mythic+ collapses to
                        -- the universal "M+" abbreviation; Overall/Raid are
                        -- localized. "Overall (...)" variants (Blood DK San'layn /
                        -- Deathbringer) collapse to the plain localized "Overall".
                        if src.showTabs and #uniqueTabs > 0 and #uniqueTabs < 3 then
                            local shortTabs = {}
                            ...
                            s = s .. " \194\183 " .. src.hex .. table.concat(shortTabs, ", ") .. "|r"
                        elseif showLabel then
                            s = s .. (showIcon and " " or "") .. src.hex .. src.abbr .. "|r"
                        end
                        -- Archon is the only source with a popularity share, and
                        -- it's the reason to consult it. A class-consolidated row
                        -- carries the highest share across that class's specs
                        -- (see ConsolidateByClass) rather than dropping it.
                        if src.showPct and rec.popularity then
                            s = s .. " |cff808080" .. rec.popularity .. "|r"
                        end
                        parts[#parts + 1] = s
                    end
                end
...
local function OnTooltipItem(tooltip, tooltipData)
    ...
    if itemId and bisScope ~= "off" and (AnyBisSourceEnabled() or ClassCodexDB.showTrinketTooltip) then
        ...
        if #entries > 0 then
            ...
            local headerSources = {}
            for _, src in ipairs(BIS_TOOLTIP_SOURCES) do
                if ClassCodexDB[src.setting] then
                    headerSources[#headerSources + 1] = BisSourceChip(src, showIcon, showLabel)
                end
            end
...
eventFrame:SetScript("OnEvent", function(_, event, arg1)
    if event == "ADDON_LOADED" and arg1 == addonName then
        local dbDefaults = {
            ...
            -- Off by default: four sources on every item tooltip is a wall of
            -- text, and these two arrived after the tooltip's shape was settled.
            showWowheadBisTooltip = false,
            showArchonBisTooltip = false,
