...
function ns:UpdateGearingSections()
    ...
    -- Same for Wowhead gear. Without this the rows render as the "Item 12345"
    -- placeholder until something else happens to pull the item into the
    -- client's cache — most Wowhead picks are current-tier drops the player has
    -- never inspected, so almost every row would show the fallback.
    local whSpecData = playerClass and playerSpec and ns:GetWowheadGearSpecData(playerClass, playerSpec)
    if whSpecData and whSpecData.bisGear then
        for _, tab in ipairs(whSpecData.bisGear) do
            for _, g in ipairs(tab.slots) do ns.RequestItemData(g.item.itemId) end
        end
    end

    -- Same prefetch for Archon: its rows are current-tier drops the player has
    -- almost certainly never inspected, so without this every row renders as the
    -- "Item 12345" placeholder.
    local _archonPre = playerClass and playerSpec and ns.GetArchonGearSpecData
        and ns:GetArchonGearSpecData(playerClass, playerSpec)
    if _archonPre and _archonPre.bisGear then
        for _, tab in ipairs(_archonPre.bisGear) do
            for _, g in ipairs(tab.slots) do ns.RequestItemData(g.item.itemId) end
        end
    end

    -- Wowhead publishes enchants/gems/consumables as item ids, so they render
    -- with real names and icons rather than u.gg's name-only enchant strings.
    local _whClass = select(2, UnitClass("player"))
    local _whSpecKey = ns.GetSpecKey and ns.GetSpecKey()
    local _whSpec = _whSpecKey and (_whSpecKey:match("-(.+)") or _whSpecKey) or nil
    local _whEnh = _whClass and _whSpec and ns.GetWowheadEnhancements
    ...
    local enchHeight, gemCount, consumCount = ns.Sections.Enhancements.RenderPanel({
        ...
        whEnchants      = _whEnh and _whEnh.enchants,
        whGems          = _whEnh and _whEnh.gems,
        whConsumables   = _whEnh and _whEnh.consumables,
        archonEnchants    = _archonEnh and _archonEnh.enchants,
        archonGems        = _archonEnh and _archonEnh.gems,
        archonConsumables = _archonEnh and _archonEnh.consumables,
    ...
    local _whSpec = _classToken and _spec and ns:GetWowheadGearSpecData(_classToken, _spec)
    local _archonSpec = _classToken and _spec and ns.GetArchonGearSpecData
        and ns:GetArchonGearSpecData(_classToken, _spec)
    lastBisCount = ns.Sections.Gear.RenderPanel({
        ivBis      = _ivSpec and _ivSpec.bisGear,
        uggBis  = _uggSpec and _uggSpec.bisGear,
        whBis      = _whSpec and _whSpec.bisGear,
