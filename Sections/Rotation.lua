...
function Rotation.InitPanel(opts)
    ...
    -- Source picker sits above the context picker. Both are optional per render
    -- (a spec with one source and one context shows neither), so their anchors
    -- are set in RenderPanel rather than here.
    panel.srcDropdown = ns.CreateOptionDropdown("ClassCodexRotSrcDropdown", panel.content)
...
function Rotation.IsPanelSrcDropdownShown() return panel.srcDropdown:IsShown() end
...
function Rotation.RenderPanel(args)
    ...
    local rowsUsed = 0
    if showSrc then
        panel.srcDropdown:ClearAllPoints()
        panel.srcDropdown:SetPoint("TOPLEFT", 0, -DROPDOWN_ROW * rowsUsed)
        panel.srcDropdown:SetPoint("TOPRIGHT", 0, -DROPDOWN_ROW * rowsUsed)
        panel.srcDropdown:Show()
        panel.srcDropdown:SetOptions(srcOptions, args.currentSource, function(picked)
            if args.onSourceChange then args.onSourceChange(picked) end
        end)
        rowsUsed = rowsUsed + 1
    else
        panel.srcDropdown:Hide()
    end
    ...
    if showCtx then
        ...
        panel.ctxDropdown:SetPoint("TOPLEFT", 0, -DROPDOWN_ROW * rowsUsed)
        panel.ctxDropdown:SetPoint("TOPRIGHT", 0, -DROPDOWN_ROW * rowsUsed)
