...
-- 12.1.0 removed the GetInspectSpecialization global and moved it under
-- C_SpecializationInfo. Resolve it per call rather than caching a reference at
-- load, so the same file works either side of the change. Returns 0 (the
-- "unknown spec" sentinel the callers already expect) when neither exists.
local function GetInspectSpecID(unit)
    local fn = (C_SpecializationInfo and C_SpecializationInfo.GetInspectSpecialization)
        or _G.GetInspectSpecialization
    return (fn and fn(unit)) or 0
end
...
local function ResolveInspectOverride()
    ...
    if unit and UnitExists and UnitExists(unit) then
        ...
        local specID = GetInspectSpecID(unit)
...
local function BuildLoadoutLabel(build)
    ...
    elseif build._whSource then source = "Wowhead"
    elseif build._archonSource then source = "Archon"
...
-- Wowhead publishes a build per (hero talent, context), so unlike Icy Veins the
-- hero is real rather than "All" — BuildLoadoutLabel then renders e.g.
-- "San'layn Raid" instead of a bare context.
local function WowheadBuildRecord(build)
    return {
        heroTalent = build.heroTalent or "All",
        context = build.context or "Build",
        buildLabel = build.buildLabel,
        exportString = build.exportString,
        recommended = false,
...
local function PopulateWowheadMenu(rootDescription)
    local classFile, specName = CurrentClassSpec()
    local builds = (classFile and specName and ns.GetWowheadTalentBuilds)
        and ns:GetWowheadTalentBuilds(classFile, specName) or nil
    if not builds or #builds == 0 then
        rootDescription:CreateTitle((ns.L and ns.L["loadout_dock.no_wowhead_builds"]) or "No Wowhead talent builds available.")
        return
    end

    local lastHero
    for _, build in ipairs(builds) do
        -- The accessor returns builds sorted by hero, so a title per hero change
        -- groups the menu without needing a second pass.
        if build.heroTalent and build.heroTalent ~= lastHero then
...
-- Archon groups by (hero talent, context) exactly as Wowhead does, so the record
-- shape is the same; only the attribution flag differs.
local function ArchonBuildRecord(build)
    return {
        heroTalent = build.heroTalent or "All",
        context = build.context or "Build",
        buildLabel = build.buildLabel,
...
local function PopulateArchonMenu(rootDescription)
    ...
    for _, build in ipairs(builds) do
        ...
        -- Archon is the only source with a popularity share, so show it — it's
        -- the whole reason to pick this source over the editorial ones.
        local label = build.buildLabel or build.context or "Build"
...
local function PopulateMenu(_, rootDescription)
    ...
    elseif selectedSource == "wowhead" then
        PopulateWowheadMenu(rootDescription)
    elseif selectedSource == "archongg" then
...
local function PopulateSourceMenu(_, rootDescription)
    ...
    -- Wowhead only covers some specs, so unlike the others it's offered per-spec
    -- rather than unconditionally — listing it everywhere would hand the player
    -- a source that renders nothing on a third of specs.
    do
        local classFile, specName = CurrentClassSpec()
        if classFile and specName and ns.HasWowheadTalents and ns:HasWowheadTalents(classFile, specName) then
            makeRadio(SOURCE_INFO.wowhead.label, "wowhead")
        elseif selectedSource == "wowhead" then
            selectedSource = "icyveins"
        end
        -- Archon is offered per-spec for the same reason as Wowhead.
        if classFile and specName and ns.HasArchonTalents and ns:HasArchonTalents(classFile, specName) then
            makeRadio(SOURCE_INFO.archongg.label, "archongg")
        elseif selectedSource == "archongg" then
            selectedSource = "icyveins"
        end
    end
...
function ns.DumpInspectState()
    ...
    if PlayerSpellsFrame then
        ...
        if unit and UnitExists(unit) then
            ...
            p("  GetInspectSpecialization:", tostring(GetInspectSpecID(unit)))
        end
    end
